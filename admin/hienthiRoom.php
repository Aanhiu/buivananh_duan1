...
if (isset($_GET['delete'])) {
    ...
    echo "<script>alert('Xóa đơn đặt phòng thành công của đơn đặt phòng này!'); window.location=lichsu_datphong.php'.php';</script>";
}
...
$query = "SELECT nguoi_dung.name AS nguoiDungTen, nguoi_dung.email, nguoi_dung.sdt, nguoi_dung.diachi, nguoi_dung.cmnd, 
          ...
          JOIN phong ON dat_phong.phong_id = phong.id WHERE dat_phong.trangthai = 7
          ORDER BY dat_phong.id DESC";

$result = mysqli_query($conn, $query);

// xu li hiện phòng lại update trạng thái thành có ko thể thuê thành có thể thuê theo id đó 

// Xử lý khi nút "Hiển thị lại Phòng" được nhấn
if (isset($_POST['hienthilairoom'])) {
    $id = $_POST['booking_id']; // Đảm bảo bạn gửi 'booking_id' qua POST
    // Truy vấn để lấy ID phòng từ bảng 'dat_phong'
    $phongQuery = "SELECT phong_id FROM dat_phong WHERE id = ?";
    $stmt = mysqli_prepare($conn, $phongQuery);
    mysqli_stmt_bind_param($stmt, "i", $id);
    mysqli_stmt_execute($stmt);
    $result = mysqli_stmt_get_result($stmt);
    $phongRow = mysqli_fetch_assoc($result);
    $phongId = $phongRow['phong_id'];

    // Cập nhật trạng thái phòng
    $updatePhongQuery = "UPDATE phong SET trangthai = 0 WHERE id = ?";
    $updateStmt = mysqli_prepare($conn, $updatePhongQuery);
    mysqli_stmt_bind_param($updateStmt, "i", $phongId);
    mysqli_stmt_execute($updateStmt);

    echo "<script>alert('Phòng đã được hiển thị lại để thuê.'); window.location='lichsu_datphong.php';</script>";
}

if (isset($_GET['hienthilairoom'])) {
    $id = $_GET['hienthilairoom'];
    $updateRoom = "UPDATE dat_phong SET trangthai = 8 WHERE id = ?"; // Trạng thái đã hiện lại room
    ...
    <div class="container-fluid" id="main-content">
        <div class="row">
            <div class="col-lg-10 ms-auto p-4 overflow-hidden">
                <h3 class="mb-4">Admin -Quản lí Hiển thi Room</h3>
                            ...
                            <table class="table table-hover border">
                                <thead class="sticky-top">
                                    <tr class="bg-dark text-light">
                                        ...
                                        <th scope="col">Hành Động / Ấn 2 lần để hiển thị</th>
                                    </tr>
                                </thead>
                                <tbody>
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                ...
                                                <?php if ($row['trangthai'] == 7) : ?>
                                                    ...
                                                    <form action="lichsu_datphong.php" method="POST">
                                                        <input type="hidden" name="booking_id" value="<?php echo $row['id']; ?>">
                                                        <button class="btn btn-warning" name="hienthilairoom">
                                                            <a href="?hienthilairoom=<?php echo $row['id']; ?>"> Hiển Thi lại Room</a>
                                                        </button>
                                                    </form>
                                                <?php endif; ?>
